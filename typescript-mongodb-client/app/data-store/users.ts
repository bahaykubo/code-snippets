...
export class Users {
  ...
  constructor() {
    ...
    this.connect();
    this.userStore = this.userStoreClient.db(Config.db.name).collection(Config.db.collections.users);
  }
  ...
  async updateSubscription(options: { name: string; newSubscription: SubsriptionType }) {
    return await this.userStore.updateOne({ name: options.name }, { $set: { subscription: options.newSubscription } });
  }
